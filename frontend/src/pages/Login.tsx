...
const Login = () => {
  ...
  const [googleAccounts, setGoogleAccounts] = useState<{name: string, email: string, avatarLetter: string}[]>([]);
...
const handleGoogleLogin = () => {
  const savedStr = localStorage.getItem("savedGoogleAccounts");
  const accounts = savedStr ? JSON.parse(savedStr) : [];
  if (accounts.length === 0) {
    setCustomEmailMode(true);
  } else {
            ...
            {!customEmailMode ? (
              <>
                ...
                {googleAccounts.map((acc) => {
                  const colors = ["#8A2BE2", "#1E90FF", "#2E8B57", "#CD5C5C", "#BA55D3", "#4682B4"];
                  const color = colors[Math.abs(acc.name.split("").reduce((acc, char) => acc + char.charCodeAt(0), 0)) % colors.length];

                  return (
                    <button
                      key={acc.email}
                      onClick={() => triggerGoogleLogin(acc.email)}
