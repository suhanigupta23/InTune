...
async function processAadhaar(file: File) {
  ...
  for (const candidate of allCandidates) {
    if (validateAadhaar(candidate)) {
      ...
      maskedAadhaar = `XXXX XXXX ${cleanCand.slice(-4)}`;
      rawAadhaar = cleanCand;
      break;
    }
  }

  return { gender, isValid, maskedAadhaar, rawAadhaar };
}
...
const handleSignup = async (e: React.FormEvent) => {
    ...
    let extractedData = { gender: "Unknown" as any, isValid: false, maskedAadhaar: "", rawAadhaar: "" };
    try {
      setOcrLoading(true);
      const ocrRes = await processAadhaar(aadhaarFile);
      extractedData.gender = ocrRes.gender;
      extractedData.isValid = ocrRes.isValid;
      extractedData.maskedAadhaar = ocrRes.maskedAadhaar;
      extractedData.rawAadhaar = ocrRes.rawAadhaar;
      ...
      if (!extractedData.isValid) {
        if (showManualAadhaar && manualAadhaar.length === 12) {
          if (validateAadhaar(manualAadhaar)) {
            ...
            extractedData.rawAadhaar = manualAadhaar;
          } else {
            toast({
