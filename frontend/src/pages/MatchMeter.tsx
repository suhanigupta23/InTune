...
import { TrendingUp, Users, Heart, MessageSquare, AlertCircle, Mic, CheckCircle, ArrowLeft } from "lucide-react";
    ...
    const fetchMatches = async () => {
      try {
        ...
        if (meData.vibeText) {
          ...
          // Alert user if a newly added highly compatible candidate is discovered
          const hasNewHighMatch = scored.some(c => (c as any).isNewMatch);
          if (hasNewHighMatch) {
            setTimeout(() => {
              toast({
                title: "🔥 New High Match Found!",
                description: "A newly registered roommate is a great vibe match for you! Look for the alert badge.",
              });
            }, 3000);
          }
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="mb-6 flex justify-start">
            <Button
              ...
              className="border-soft-sand text-warm-brown hover:bg-light-cream font-semibold rounded-xl px-4 py-2 text-sm flex items-center gap-2"
            >
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-1 gap-6">
                  {candidates.slice(0, 5).map((c, index) => {
                    const matchMockFormat = {
                      ...
                      chatroom_passkey: c._id,
                      isNewMatch: (c as any).isNewMatch
                    };
                    return (
                      <MatchCard
                        key={c._id}
