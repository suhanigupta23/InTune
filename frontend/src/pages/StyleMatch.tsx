...
import { ArrowLeft, Sparkles, Send, Vote, Check, Eye, Heart, Lock, ShieldAlert } from "lucide-react";
...
const StyleMatch = () => {
  ...
  const [matchedRoommate, setMatchedRoommate] = useState<any>(null);
  ...
  useEffect(() => {
    ...
    const fetchRoommateMatches = async () => {
      try {
        ...
        const res = await fetch(`${API_BASE}/auth/matches`, {
          headers: { Authorization: `Bearer ${token}` }
        });
        if (res.ok) {
          const data = await res.json();
          if (data && data.length > 0) {
            setMatchedRoommate(data[0]); // Fetch real matched roommate
          }
        }
      } catch (err) {
        console.error("Error loading roommate matches:", err);
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <main className="flex-1 max-w-6xl w-full mx-auto p-4 md:py-8 flex flex-col justify-center">
        {/* Back Button */}
        <Button 
          variant="ghost" 
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-20 space-y-4">
            <div className="w-8 h-8 border-4 border-warm-brown border-t-transparent rounded-full animate-spin"></div>
            <p className="text-sm text-muted-text">Verifying roommate session status...</p>
          </div>
        ) : !isLoggedIn ? (
          /* Locked State: Authenticated check */
          <div className="max-w-md mx-auto my-12 w-full">
            <Card className="border border-soft-sand bg-gradient-to-br from-warm-white to-light-cream shadow-2xl p-8 rounded-2xl text-center space-y-6">
              <div className="mx-auto w-16 h-16 bg-red-100 rounded-full flex items-center justify-center text-red-600 animate-pulse">
                <Lock className="w-8 h-8" />
              </div>
              <div className="space-y-2">
                <CardTitle className="text-2xl font-bold text-warm-brown">StyleMatch Locked</CardTitle>
                <CardDescription className="text-muted-text text-sm">
                  You must be logged in to collaborate on room theme layouts and use the StyleMatch AI Assistant.
                </CardDescription>
              </div>
              <Button 
                onClick={() => navigate("/login")}
                className="w-full bg-warm-brown hover:bg-warm-brown-dark text-white font-bold py-3 rounded-xl shadow-lg transition-smooth"
              >
                Log In to Continue
              </Button>
            </Card>
          ...
          /* Locked State: Roommate Match check */
          <div className="max-w-md mx-auto my-12 w-full">
            <Card className="border border-soft-sand bg-gradient-to-br from-warm-white to-light-cream shadow-2xl p-8 rounded-2xl text-center space-y-6">
              ...
              <div className="space-y-2">
                <CardTitle className="text-2xl font-bold text-warm-brown">Roommate Match Required</CardTitle>
                <CardDescription className="text-muted-text text-sm">
                  StyleMatch allows you to co-design layouts with your matched roommate. Please find and lock a roommate match on the Match Meter first!
                </CardDescription>
              </div>
              <Button 
                ...
                className="w-full bg-warm-brown hover:bg-warm-brown-dark text-white font-bold py-3 rounded-xl shadow-lg transition-smooth"
              >
                Find Roommates on Match Meter
              </Button>
            </Card>
          </div>
        ) : (
          /* Unlocked Collaborative Designer Page */
          <>
            {/* Page Header */}
            <div className="mb-8">
                      ...
                      <div className="p-4 border-t border-soft-sand flex flex-col gap-2 bg-light-cream/35">
                        ...
                        {template.roommateVoted && (
                          <div className="text-xxs text-warm-brown font-semibold flex items-center gap-1 bg-amber-50 border border-soft-sand px-2.5 py-1 rounded">
                            ...
                            {activeRoommate.name || activeRoommate.anonymousId} voted for this!
                          </div>
                        )}
